...
#if 0
// FIXME: Unprintable characters appearing (only now?), artifact of CopyMem alignment?
// Alternatively, requires PACKED?
  ...
  if (DebugLogsRingbuffer->Header.Cursor == DebugLogsRingbuffer->Header.Size*SIZE_1KB) {
    // Will overflow next message, zero cursor.
    DebugLogsRingbuffer->Header.Cursor = 0;
  }
  ...
  do {
    ...
    DebugLogsRingbuffer->Body[DebugLogsRingbuffer->Header.Cursor++] = Buffer[Sent++];
