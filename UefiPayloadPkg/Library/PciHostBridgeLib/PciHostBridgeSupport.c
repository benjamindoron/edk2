...
)
{
  ...
  for (Index = 0; Index < NumberOfDescriptors; Index++) {
    ...
    if (Map[Index].GcdMemoryType == EfiGcdMemoryTypeReserved) {
      DEBUG ((DEBUG_ERROR, "%a: PCI aperature overlaps reserved memory region\n",
        __FUNCTION__));
      DEBUG ((DEBUG_ERROR, "%a: Please consider fixing EDK2 or your firmware\n",
        __FUNCTION__));
    ...
    if ((Value & BIT0) == BIT0) {
      ...
      if ((Command & EFI_PCI_COMMAND_MEMORY_SPACE) != 0) {
        ...
        if ((Base > 0) && (Base < Limit)) {
          ...
          if (EFI_ERROR (Status)) {
            DEBUG ((DEBUG_ERROR, "%a: Not adding PCI %x:%x.%x BAR@%d %x:%x to aperature\n",
              __FUNCTION__,
              Bus,
              Device,
              Function,
