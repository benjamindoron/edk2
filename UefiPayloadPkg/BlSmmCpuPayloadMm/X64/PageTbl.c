  ...
  )
{
  ...
  DEBUG ((DEBUG_INFO, "5LevelPaging Needed             - %d\n", gPayloadMmCpuPrivateData->PayloadMmPrivateData.Intel5LevelPagingNeeded));
      ...
      NULL_DETECTION_NONSTOP_MODE)
  {
    ...
    IdtEntry                    = (IA32_IDT_GATE_DESCRIPTOR *)gSmiHandlerIdtr.Base;
  ...
  if (FeaturePcdGet (PcdCpuSmmStackGuard)) {
    ...
    // BUGBUG: This should be `1`, but we don't actually have a TSS because we're using coreboot's GDT.
    // TODO: Make our own GDT after all? And since AP's stacks must be preserved when trading places, multiple *are* required.
    InitializeIdtIst (EXCEPT_IA32_PAGE_FAULT, 0);
  }
  ...
  )
{
  ...
  //
  if ((PFAddress >= mSmrrBase) &&
      (PFAddress < (mSmrrBase + mSmrrSize)))
  {
    DumpCpuContext (InterruptType, SystemContext);
