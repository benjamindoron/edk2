  ...
  )
{
  ...
  if ((Attributes & EFI_MEMORY_XP) != 0) {
    if (gPayloadMmCpuPrivateData->PayloadMmPrivateData.XdSupported) {
  ...
  if ((Attributes & EFI_MEMORY_RP) != 0) {
    if (IsSet) {
      ...
      DEBUG_CODE_BEGIN ();
      if (((Attributes & EFI_MEMORY_RO) == 0) || (((Attributes & EFI_MEMORY_XP) == 0) && (gPayloadMmCpuPrivateData->PayloadMmPrivateData.XdSupported))) {
        //
        ...
        for (Index = 0; Index < Count; Index++) {
          ...
          if ((BaseAddress < Map[Index].LinearAddress + Map[Index].Length) && (BaseAddress + Length > Map[Index].LinearAddress)) {
            ...
            if (((Attributes & EFI_MEMORY_XP) == 0) && (gPayloadMmCpuPrivateData->PayloadMmPrivateData.XdSupported) && (Map[Index].Attribute.Bits.Nx == 1)) {
  ...
  )
{
  ...
  for (Index = 0; Index < mMaxNumberOfCpus; Index++) {
    ...
    // Data
    //
    if (gPayloadMmCpuPrivateData->SmmCoreEntryContext.CpuSaveStateSize[Index] != 0) {
      SmmClearMemoryAttributes (
        (EFI_PHYSICAL_ADDRESS)gPayloadMmCpuPrivateData->SmmCoreEntryContext.CpuSaveState[Index],
        gPayloadMmCpuPrivateData->SmmCoreEntryContext.CpuSaveStateSize[Index],
        EFI_MEMORY_RO
        );
      SmmSetMemoryAttributes (
        (EFI_PHYSICAL_ADDRESS)gPayloadMmCpuPrivateData->SmmCoreEntryContext.CpuSaveState[Index],
  ...
  )
{
  ...
  //
  // GDT protected by InitGdt(), which maps and protects bootloader's code.
  //
  ...
  BaseAddress = gSmiHandlerIdtr.Base;
  Size        = ALIGN_VALUE (gSmiHandlerIdtr.Limit + 1, SIZE_4KB);
